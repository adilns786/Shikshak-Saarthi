# PBAS Server (Flask) — _server

This folder contains a small Flask server used by the Shikshak Sarthi project.

Quick tasks performed by me:
- Guarded ReportLab imports in `main.py` so the module imports even when `reportlab` is missing.
- Removed a duplicate `/health` route that raised a Flask endpoint collision.

Quick start (Windows)
1. Activate the project's virtual environment (this repository includes `_server/env`):
...
Notes
- The `requirements.txt` already includes `reportlab`. If you need PDF generation endpoints, ensure the packages are installed into the same Python environment used to run `main.py`.
- I left safe fallbacks in `main.py` so importing the module won't fail if `reportlab` is absent; the actual PDF functions are skipped unless `reportlab` is installed.

If you want, I can:
- Install the requirements into the included `_server/env` virtualenv and restart the server now.
- Add a tiny unit test to verify `/health` and a CI job to smoke-test the server.
