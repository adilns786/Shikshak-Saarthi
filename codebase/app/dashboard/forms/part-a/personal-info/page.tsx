...
export default function PersonalContactModule() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const data = docSnap.exists()
          ? docSnap.data()?.part_a?.personal_in || {}
          : {};
        ...
        personalContactFields.forEach((field) => {
          if (!data[field.id] && llmStorage[field.id]) {
            ...
            data[field.id] &&
            llmStorage[field.id] &&
            data[field.id] !== llmStorage[field.id]
          ) {
  ...
  const handleUndoSuggestion = (id: string) => {
    setSuggestions((prev: Record<string, any>) => {
      const newS = {
        ...prev,
        [id]: JSON.parse(localStorage.getItem("llm") || "{}")[id],
      };
      return newS;
    });
  };
  ...
  const handleApplyAll = () => {
    ...
    setPersonalIn((prev: any) => ({ ...prev, ...allSuggestions }));
    setSuggestions({});
  };
              ...
              <input
                ...
                className={`p-2 border rounded-md ${
                  editing
                    ...
                    : "border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-3)] cursor-default"
                }`}
              />
