...
export default function EmploymentHistoryModule() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const firestorePrior = docSnap.exists()
          ? docSnap.data()?.part_a?.employment?.prior || []
          : [];
        const firestorePosts = docSnap.exists()
          ? docSnap.data()?.part_a?.employment?.posts || []
          : [];
        ...
        const newPriorSuggestions = localPrior.filter(
          (row: any) =>
            !firestorePrior.some(
              (r: any) => JSON.stringify(r) === JSON.stringify(row),
            ),
        );
  ...
  const saveToFirestore = async (
    ...
    try {
      ...
      localStorage.setItem(
        "llm",
        JSON.stringify({ ...llmStorage, [key]: updated }),
      );
  ...
  const renderTable = (
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full table-auto border border-[var(--border-default)] rounded-md mb-4">
        <thead>
    ...
    suggestions.length > 0 && (
      <div className="mt-3 border border-yellow-300 rounded-lg bg-yellow-50 p-3">
        <h4 className="font-semibold text-yellow-700 mb-2">
          Suggested New {section === "prior" ? "Prior Appointments" : "Posts"}{" "}
          (from previous data)
        </h4>
  ...
  return (
    <div className="space-y-4">
      <Card className="w-full max-w-5xl mx-auto shadow-xl rounded-2xl">
