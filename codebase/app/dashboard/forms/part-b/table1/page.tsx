...
export default function TeachingStudentAssessmentModule() {
  ...
  const [suggestedTeachingRows, setSuggestedTeachingRows] = useState<
    SuggestedRow[]
  >([]);
  const [suggestedActivityRows, setSuggestedActivityRows] = useState<
    SuggestedRow[]
  >([]);
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          llmTeaching.forEach((llmRow: any) => {
            ...
            const existsInFirestore = firestoreTeaching.some(
              (fsRow: any) =>
          ...
          llmActivities.forEach((llmRow: any) => {
            const existsInFirestore = firestoreActivities.some(
              (fsRow: any) =>
  ...
  const undoSuggestedTeachingRow = (index: number) => {
    ...
    const updated = teachingRows.filter(
      (row) =>
        !(
  ...
  const undoSuggestedActivityRow = (index: number) => {
    ...
    const updated = activityRows.filter(
      (row) =>
        !(
  ...
  const applyAllSuggestions = () => {
    ...
    // Mark all as applied
    setSuggestedTeachingRows(
      suggestedTeachingRows.map((s) => ({ ...s, applied: true })),
    );
    setSuggestedActivityRows(
      suggestedActivityRows.map((s) => ({ ...s, applied: true })),
    );
  };

  const totalSuggestions =
    suggestedTeachingRows.filter((s) => !s.applied).length +
        ...
        <CardContent>
          ...
          <div className="mb-8">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border border-[var(--border-default)] rounded-md">
        ...
        <div key={f.id} className="flex flex-col">
          ...
          {f.type === "select" ? (
            <select
              ...
              className="p-2 border rounded-md border-[var(--border-default)] focus:outline-none focus:ring-2 focus:ring-primary bg-[var(--surface-base)] text-[var(--text-1)]"
            >
