...
function toRowArray(value: any): Row[] {
...
function mergeUniquePublications(primary: Row[], secondary: Row[]): Row[] {
  const merged: Row[] = [];
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          const userPublications = toRowArray(
            tbl.publications ?? tbl.publication ?? tbl.booksPublications,
          );
          ...
          let mergedPublications = userPublications;
          try {
            const pbasSnap = await getDocs(
              query(
                collection(firestore, "pbas_forms"),
                where("user_id", "==", user.uid),
              ),
            );

            const legacyPublications: Row[] = [];
            pbasSnap.docs.forEach((d) => {
              const legacyTable2 = d.data()?.part_b?.table2 || {};
              legacyPublications.push(
                ...toRowArray(
                  legacyTable2.publications ??
                    legacyTable2.publication ??
                    legacyTable2.booksPublications,
                ),
              );
            });

            mergedPublications = mergeUniquePublications(
              userPublications,
              legacyPublications,
            );
          } catch {
            // Legacy source is optional; continue with users table2 data.
          }
        ...
        <CardContent>
          {rows.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full table-auto border border-[var(--border-default)] rounded-md">
                <thead>
                  ...
                  getDoc(doc(firestore, "users", userId))
                    .then(async (snap) => {
                      if (snap.exists()) {
                        ...
                        const userPublications = toRowArray(
                          t.publications ?? t.publication ?? t.booksPublications,
                        );
                        ...
                        try {
                          ...
                          // ignore optional legacy source failures
                        }
