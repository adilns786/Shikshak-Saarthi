...
export default function FacultyStatsPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const roleDistribution = Array.from(roleMap.entries()).map(
        ([role, count]) => ({
  ...
  const COLORS = ["#FF5C35", "#7C5CFC", "#00C48C", "#FFAA00", "#007AFF"];
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="w-full sm:w-auto">
            ...
            <p className="text-sm mt-1" style={{ color: "var(--text-3)" }}>
              Overall performance analytics and trends
            </p>
          </div>
        </div>
      </header>
  ...
  return (
    <AppShell
      user={{
        ...
        role: (currentUser?.role ?? "faculty") as any,
