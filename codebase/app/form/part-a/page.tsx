...
export default function PartAForm() {
  ...
  const removeTableRow = (subsectionId: string, rowIndex: number) => {
    setFormData((prev) => ({
      ...prev,
      [subsectionId]: (prev[subsectionId] || []).filter(
  ...
  const validateForm = () => {
    ...
    (formJson.sections[0]?.subsections ?? []).forEach(
        ...
        if (subsection.fields) {
          subsection.fields.forEach((field: Field) => {
            if (field.required) {
              ...
              if (!value) {
                newErrors[`${subsection.id}.${field.id}`] =
        ...
        return (
          <div key={field.id} className="space-y-2">
            ...
            <textarea
              value={value}
              onChange={(e) =>
                handleInputChange(subsection.id, field.id, e.target.value)
              }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8 px-4">
      ...
      >
            ...
            {formJson.formHeader.fields.map((field) => (
              <div key={field.id} className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("header", field.id, e.target.value)
                  }
        ...
        <div ref={formRef} className="space-y-6">
          {(formJson.sections[0]?.subsections ?? []).map(
