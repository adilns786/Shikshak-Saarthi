...
function generateResearchPapers(count, name) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    papers.push({
      ...
      journal: isJournal
        ? journals[getRandomInt(0, journals.length - 1)]
        : conferences[getRandomInt(0, conferences.length - 1)],
      year: getRandomYear(),
...
function generatePublications(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    pubs.push({
      title: isBook
        ? `Book on ${["AI", "ML", "Data Science", "IoT"][getRandomInt(0, 3)]}`
...
function generatePatentsAwards(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    items.push({
      title: isPatent
        ? `Patent ${i + 1} on Innovation`
        : `Award ${i + 1} for Excellence`,
      ...
      recognition: isPatent
        ? "Government of India - IPO"
        : ["Institution", "State", "National"][getRandomInt(0, 2)],
      application_no: isPatent
        ? `${getRandomYear()}${getRandomInt(10000000, 99999999)}`
        : "",
...
function generateLectures(count) {
  ...
  for (let i = 0; i < count; i++) {
    lectures.push({
      ...
      organizer: ["IEEE", "ACM", "Institution", "Industry Partner"][
        getRandomInt(0, 3)
      ],
      venue: ["Mumbai", "Pune", "Bangalore", "Delhi", "Online"][
        getRandomInt(0, 4)
      ],
      date: `${getRandomYear()}-${String(getRandomInt(1, 12)).padStart(2, "0")}-${String(getRandomInt(1, 28)).padStart(2, "0")}`,
      type: ["Invited Talk", "Keynote", "Workshop", "Seminar"][
        getRandomInt(0, 3)
      ],
    });
  }
...
function generateProjects(count) {
  ...
  for (let i = 0; i < count; i++) {
    projects.push({
      ...
      funding_agency:
        fundingAgencies[getRandomInt(0, fundingAgencies.length - 1)],
...
function generateCompleteData(name, department, role) {
  ...
  return {
    formHeader: {
      ...
      cas_promotion_stage:
        role === "hod" ? "Stage 2 (Senior Scale)" : "Stage 1",
    },
    part_a: {
      academic_qualifications: [
        ...
        },
        {
          ...
          board_university: ["IIT Bombay", "IIT Delhi", "University of Mumbai"][
            getRandomInt(0, 2)
          ],
      ...
      personal_in: {
        ...
        department: department,
        designation_applied:
          role === "hod" ? "Professor" : "Assistant Professor (Senior Scale)",
      ...
      courses_fdp: Array.from({ length: getRandomInt(3, 6) }, (_, i) => ({
        ...
        organizer: ["IIT Bombay", "AICTE", "NPTEL", "Industry Partner"][
          getRandomInt(0, 3)
        ],
      ...
      teaching_student_assessment: {
        teaching: Array.from({ length: getRandomInt(2, 4) }, (_, i) => ({
          ...
          self_appraisal: ["Excellent", "Very Good", "Good"][
            getRandomInt(0, 2)
          ],
          verified_grading: ["Excellent", "Very Good", "Good"][
            getRandomInt(0, 2)
          ],
        })),
    ...
    part_b: {
      ...
      table2: {
        ...
        consultancyProjects: Array.from(
  ...
  try {
    ...
    for (const userDoc of querySnapshot.docs) {
      ...
      const hasQualifications =
        userData.part_a?.academic_qualifications?.length > 0;
      ...
      const completeData = generateCompleteData(
        ...
        userData.department ||
          departments[getRandomInt(0, departments.length - 1)],
      ...
      console.log(
        `   • Research Papers: ${completeData.part_b.table2.researchPapers.length}`,
      );
      console.log(
        `   • Publications: ${completeData.part_b.table2.publications.length}`,
      );
      console.log(
        `   • Patents/Awards: ${completeData.part_b.patents_policy_awards.length}\n`,
      );
    }
