...
export default function HODDashboard() {
  ...
  const [selectedTab, setSelectedTab] = useState<
    "overview" | "faculty" | "analytics" | "reports"
  >("overview");
  ...
  const generateAIInsights = (
    ...
    const pubPerFaculty =
      stats.totalFaculty > 0 ? stats.totalPublications / stats.totalFaculty : 0;
    if (pubPerFaculty >= 3) {
      insights.push(
        `🌟 Excellent publication rate! Your department averages ${pubPerFaculty.toFixed(1)} publications per faculty member.`,
      );
    } else if (pubPerFaculty >= 1) {
      insights.push(
        `📊 Good publication rate of ${pubPerFaculty.toFixed(1)} per faculty. Consider organizing writing workshops to boost output.`,
      );
    } else {
      insights.push(
        `📝 Publication rate is ${pubPerFaculty.toFixed(1)} per faculty. Encourage collaborative research to improve.`,
      );
    }
    ...
    if (stats.totalProjects > stats.totalFaculty) {
      insights.push(
        `💼 Strong project portfolio with ${stats.totalProjects} active projects across the department.`,
      );
    }
    ...
    if (stats.totalFunding > 1000000) {
      insights.push(
        `💰 Impressive funding of ₹${(stats.totalFunding / 100000).toFixed(2)} Lakhs secured by the department.`,
      );
    } else if (stats.totalFunding > 0) {
      insights.push(
        `💡 Consider applying for more major research grants. Current funding: ₹${(stats.totalFunding / 100000).toFixed(2)} Lakhs.`,
      );
    }
    ...
    if (stats.appraisalsPending > 0) {
      insights.push(
        `⏰ ${stats.appraisalsPending} faculty members have pending appraisals. Send reminders for timely submission.`,
      );
    } else {
      insights.push(
        `✅ All faculty appraisals are submitted. Great department coordination!`,
      );
    }
    ...
    if (stats.totalPatents > 0) {
      insights.push(
        `🏆 Department has ${stats.totalPatents} patent(s). Innovation is thriving!`,
      );
    } else {
      insights.push(
        `💡 No patents filed yet. Consider organizing an IP awareness session.`,
      );
    }
    ...
    return (
      <div className="page-shell flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="w-12 h-12 rounded-full border-2 border-[var(--brand-primary)] border-t-transparent animate-spin mx-auto" />
  ...
  const publicationTypeData = facultyMembers.reduce(
    (acc: { name: string; value: number }[], faculty) => {
      ...
      return [
        {
          ...
          value:
            (acc.find((a) => a.name === "Research Papers")?.value || 0) +
            papers,
        },
        {
          name: "Publications",
          value:
            (acc.find((a) => a.name === "Publications")?.value || 0) +
            publications,
        },
        {
          name: "Conferences",
          value:
            (acc.find((a) => a.name === "Conferences")?.value || 0) +
            conferences,
        },
      ];
    },
    [
      ...
      { name: "Conferences", value: 0 },
    ],
  );

  return (
    <AppShell user={appUser} onSignOut={handleLogout}>
            ...
            >
              {["overview", "faculty", "analytics", "reports"].map((tab) => (
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium capitalize rounded-lg transition-all duration-150"
                  style={
                    selectedTab === tab
                      ? {
                        ...
                        <tbody className="divide-y divide-border">
                          {facultyMembers.map((faculty) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-3 text-sm text-foreground">
                                {(faculty.part_b?.table2?.researchPapers
                                  ?.length || 0) +
                                  (faculty.part_b?.table2?.publications
                                    ?.length || 0)}
                              </td>
                              <td className="px-4 py-3 text-sm text-foreground">
                                {(faculty.part_b?.table2?.researchProjects
                                  ?.length || 0) +
                                  (faculty.part_b?.table2?.consultancyProjects
                                    ?.length || 0)}
                              </td>
                              <td className="px-4 py-3">
                          ...
                          <AreaChart
                            data={[
                              ...
                              },
                              {
                                year: "2025",
                                publications:
                                  departmentStats?.totalPublications || 40,
                                projects: departmentStats?.totalProjects || 18,
                                patents: departmentStats?.totalPatents || 6,
                              },
                            ]}
                          >
                            <CartesianGrid strokeDasharray="3 3" />
                  ...
                  <CardContent>
                    <div className="grid gap-4">
                      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/30 transition-colors">
                        ...
                        <ExportReportButton
                          department={userData?.department}
                          targetName={`${userData?.department ?? ""} – PBAS Summary`}
                          label="Generate"
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/30 transition-colors">
                        ...
                        <ExportReportButton
                          department={userData?.department}
                          targetName={`${userData?.department ?? ""} – API Scores`}
                          label="Generate"
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/30 transition-colors">
                        ...
                        <ExportReportButton
                          department={userData?.department}
                          targetName={`${userData?.department ?? ""} – Research`}
                          label="Generate"
                        />
                      </div>
                    </div>
      ...
      <Card className="shadow-lg overflow-hidden">
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`h-14 w-14 rounded-xl ${color} flex items-center justify-center shadow-lg`}
            >
