...
const FADE_UP = {
  ...
  show: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { duration: 0.4, delay: i * 0.06, ease: [0, 0, 0.2, 1] },
  }),
};
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // Try to fetch from pbas_forms nested data
        try {
          const pbas = await getDocs(
            query(
          ...
          pbas.docs.forEach((d) => {
            ...
            raw.forEach((p: Record<string, unknown>, idx: number) => {
              pubs.push({
                id: `${d.id}-${idx}`,
                ...
                journal: (p.journal_name ??
                  ...
                  p.publisher ??
                  p.journal ??
                  p.conference) as string,
                year: (p.year ?? p.year_of_publication) as string,
                type: (p.type ??
                  (p.journal_name ? "journal" : "conference")) as string,
